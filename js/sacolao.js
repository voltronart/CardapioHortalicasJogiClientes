// ==========================================
// loja-sacolao.js - Cardápio público de sacolão
// ==========================================
// Cópia da loja-avulso.js, com uma diferença: usa a coluna
// preco_sacolao (em vez de preco) da tabela produtos_avulso,
// e chama a function criar_pedido_sacolao.
// Nome, telefone e endereço só são pedidos na hora de finalizar.

const CONFIG = {
  whatsappNumero: "5561996433209",
  pixChave: "COLOQUE_AQUI_A_CHAVE_PIX",
  pixNome: "COLOQUE_AQUI_O_NOME_DO_TITULAR",
};

let produtosDisponiveis = [];
let carrinho = {}; // { produtoId: { produto, quantidade } }

const CHAVE_CARRINHO_STORAGE = "carrinho_sacolao";
...
async function carregarProdutos() {
  ...
  // Usa o preço específico de sacolão. Produtos sem preco_sacolao
  // definido não aparecem neste cardápio (evita vender sem preço certo).
  produtosDisponiveis = (data || [])
    .filter((p) => p.preco_sacolao != null)
    .map((p) => ({ ...p, preco: p.preco_sacolao }));
...
async function finalizarPedido(event) {
  ...
  try {
    ...
    // O total NÃO é enviado - a function "criar_pedido_sacolao" recalcula
    // no servidor usando o preco_sacolao de cada produto, por segurança.
    const { data: pedidoId, error } = await supabaseClient.rpc("criar_pedido_sacolao", {
      p_nome: nome,
      p_telefone: telefone,
      p_endereco: endereco,
...
function abrirWhatsApp(itens, total, pedidoId, nomeCliente, endereco, formaPagamento, trocoPara) {
  ...
  let mensagem = `📋 *NOVO PEDIDO SACOLÃO #${numeroPedido}*\n`;
